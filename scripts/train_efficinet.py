...
from pytraction.net.dataloader import (Dataset, get_preprocessing,
...
x_train, x_valid, y_train, y_valid = train_test_split(
    x_images, y_images, test_size=0.20, random_state=42
)
...
if TRAIN:
    ...
    ax[1].plot(
        logs["epoch"], logs["train"]["jaccard_loss"], label="train", color="blue"
    )
    ax[1].plot(
        logs["epoch"], logs["valid"]["jaccard_loss"], label="valid", color="orange"
    )
    ...
    for i in range(0, 40):
        ...
        ax[0].plot(
            logs["epoch"], logs["train"]["iou_score"], label="train", color="blue"
        )
        ax[0].plot(
            logs["epoch"], logs["valid"]["iou_score"], label="valid", color="orange"
        )
        ax[0].set_xlabel("Epoch")
        ax[0].set_ylabel("IoU score")
