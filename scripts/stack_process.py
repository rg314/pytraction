...
# for frame in :
    ...
    io.imsave(f'test{frame}.tif',img)
    ...
    if TFM:
        ...
        im = ax.imshow(img, interpolation='bicubic', cmap='jet',extent=[x.min(), x.max(), y.min(), y.max()], vmin=0, vmax=5)
        ax.quiver(x, y, u, v)

        divider = make_axes_locatable(ax)
        cax = divider.append_axes("right", size="5%", pad=0.05)
        plt.colorbar(im, cax=cax)
        plt.savefig(f'output/test{frame}.png')
        plt.close()
